...
create_smooth_brace <- function(x1, y1, x2, y2, width = NULL, n_points = 100) {
  ...
  p0y <- 0 # Start point
  p1x <- 2
  p1y <- 1 # Control point 1
  p2x <- 3
  p2y <- 2 # Control point 2
  p3x <- 4
  p3y <- 2 # End point
  ...
  p0y <- 2 # Start point
  p1x <- 16
  p1y <- 2 # Control point 1
  p2x <- 17
  p2y <- 3 # Control point 2
  p3x <- 17
  p3y <- 4 # End point
...
# with semi circle ----
create_stretched_semicircle <- function(
  point1,
  point2,
  n_points = 20,
  x_stretch = 2,
...
polygon_data <- data %>%
  ...
  mutate(
    ...
    wrap_points = purrr::map(needs_wrap, function(wrap) {
      if (wrap) {
        outer_curve_points <- create_semicircle(
          c(cut_value - 1500, -bar_width / 2 - same_bar_dist - bar_width),
          c(cut_value - 1500, bar_width / 2),
          orientation = "below",
          n_points = 100,
          y_stretch = 4500
        )
        inner_curve_points <- create_semicircle(
          c(cut_value - 1500, -bar_width / 2),
        ...
        tibble(
          x = c(
            0,
            outer_curve_points$x,
            Count - cut_value,
            Count - cut_value,
            inner_curve_points$x,
            0
          ),
          y_offset = c(
            bar_width / 2,
            outer_curve_points$y,
            -bar_width / 2 - same_bar_dist - bar_width,
            -bar_width / 2 - same_bar_dist,
            inner_curve_points$y,
            -bar_width / 2
          ),
          point_order = 1:124
        )
...
bracket_polygon <- data[8:nrow(data), ] %>%
  ...
  mutate(
    wrap_points = list(tibble(
      x = c(
        shift_x_bracket_polygon,
        Count + shift_x_bracket_polygon,
        Count + shift_x_bracket_polygon,
        shift_x_bracket_polygon
      ),
      y_offset = c(
...
ggplot(polygon_data) +
  ...
  geom_path(
    data = create_smooth_brace(x1 = 500, y1 = -1.5, x2 = 500, y2 = 14.5),
    aes(x + (x - 500) * 2400, y),
    linewidth = .1
  ) +
  geom_text(
    aes(x = 26000, y = 11, label = "1890."),
    size = 10,
    family = "Teko",
  ) +
  geom_polygon(
    data = bracket_polygon,
    aes(x = x, y = y),
    fill = '#d93d51',
    color = "black",
    linewidth = .1
